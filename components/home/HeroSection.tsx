...
export default function HeroSection() {
  ...
  return (
    <>
      {/* ── 데스크탑 Hero — d933990 완전 동일 ── */}
      <section
        ref={sectionRef}
        className="relative w-full h-screen min-h-[700px] overflow-hidden hidden md:block"
      ...
      >
        ...
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(170deg, rgba(13,12,10,0) 0%, rgba(13,12,10,0.73) 55%, rgba(13,12,10,0.94) 100%)' }} />
        <AnimatePresence mode="wait">
          ...
          >
            <h1 className="font-light" style={{ fontFamily: SERIF, fontSize: 'clamp(52px, 6.94vw, 100px)', color: '#FFFFFF', lineHeight: 1.3, marginBottom: '30px' }}>
            ...
            <motion.p
              ...
              style={{ fontFamily: SERIF, fontSize: 'clamp(14px, 1.39vw, 20px)', color: 'rgba(255,255,255,0.38)', letterSpacing: '8px', lineHeight: 1, paddingLeft: '10px' }}
            >
              {slide.sub}
            </motion.p>
          </motion.div>
        </AnimatePresence>
        <motion.div className="absolute" style={{ right: '100px', top: '87.5%', width: '380px' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.0, delay: 3.2 }}>
        ...
        <motion.div className="absolute flex items-center" style={{ left: '52px', bottom: '8%', gap: '10px' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 3.0 }}>
        ...
        <motion.div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 3.3 }} style={{ bottom: '5.2%', gap: '6px' }}>
