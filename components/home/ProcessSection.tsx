...
const DEFAULT_COPY = {
  eyebrow: 'THE PROCESS',
  title: '장인의 혼을 담아',
  desc: '나무를 선별하고, 결을 읽고, 깎고 이어 붙이는 모든 과정.\n3대 장인의 손끝에서 전통건축의 혼이 담깁니다.',
  steps: ['목재 선별', '치목 (治木)', '가조립 검증', '설치 및 마감'] as [string, string, string, string],
};

const DEFAULT_TILES = [
  'https://images.unsplash.com/photo-1703541585777-5b9349aa4f6f?auto=format&fit=crop&w=800&q=80',
  'https://images.unsplash.com/photo-1564067018527-8584aca23a04?auto=format&fit=crop&w=800&q=80',
  'https://images.unsplash.com/photo-1777589555761-c7bc2a082982?auto=format&fit=crop&w=800&q=80',
  'https://images.unsplash.com/photo-1758696654625-29937273a271?auto=format&fit=crop&w=800&q=80',
];

export default function ProcessSection({ images, copy }: { images?: string[]; copy?: Partial<typeof DEFAULT_COPY> }) {
  const tiles = DEFAULT_TILES.map((def, i) => (images?.[i] && images[i] !== '') ? images[i] : def);
  const c = { ...DEFAULT_COPY, ...copy };
  const [descL1, descL2] = c.desc.split('\n');
  const steps = STEP_NUMS.map((num, i) => ({ num, title: c.steps[i] }));
    ...
    <>
      ...
      <section className="hidden md:flex" style={{ height: '600px' }}>
        <motion.div className="flex flex-col"
          ...
          <span style={{ fontFamily: SANS, fontSize: '9px', color: 'rgba(255,255,255,0.3)', letterSpacing: '4px', textTransform: 'uppercase' }}>{c.eyebrow}</span>
          <h2 className="font-light" style={{ fontFamily: SERIF, fontSize: '48px', color: '#FFFFFF', lineHeight: 1.354, width: '380px' }}>{c.title}</h2>
          <p style={{ fontFamily: SANS, fontSize: '14px', color: 'rgba(255,255,255,0.45)', lineHeight: 1.9, width: '360px' }}>
            {descL1}{descL2 && <><br />{descL2}</>}
          </p>
        ...
        <div className="flex-1 overflow-hidden" style={{ backgroundColor: '#1A1816', display: 'grid', gridTemplateColumns: '480fr 478fr', gridTemplateRows: '298px 300px', gap: '2px' }}>
          {tiles.map((url, i) => (
            <div key={i} className="relative overflow-hidden" style={{ backgroundColor: '#1A1816' }}>
      ...
      <section className="md:hidden flex flex-col" style={{ backgroundColor: '#111111', padding: '64px 24px', gap: 32 }}>
        <div className="flex flex-col" style={{ gap: 10 }}>
          <span style={{ fontFamily: SANS, fontSize: 10, letterSpacing: 3, color: 'rgba(255,255,255,0.3)' }}>{c.eyebrow}</span>
          <h2 style={{ fontFamily: SERIF, fontSize: 32, fontWeight: 300, color: '#FFFFFF' }}>{c.title}</h2>
