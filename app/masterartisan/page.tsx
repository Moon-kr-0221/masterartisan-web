  ...
  return (
    <div style={{ backgroundColor: '#FFFFFF' }}>
      ...
      <div className="hidden md:block" style={{ paddingTop: 72 }}>
          ...
          return (
            <section key={artisan.generation} style={{ backgroundColor: bg }}>
              <div className={`flex flex-col ${reversed ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
                <div className="w-full md:w-[600px] md:flex-shrink-0 overflow-hidden" style={{ height: 520, backgroundColor: '#EDEAE4' }}>
                ...
                <ScrollReveal direction={reversed ? 'left' : 'right'} className="flex-1 flex flex-col" style={{ padding: '72px 80px' }}>
                  <div className="flex flex-col gap-6">
                    <span style={{ fontFamily: SANS, fontSize: 10, letterSpacing: '3px', color: '#AAAAAA' }}>{artisan.generationEn}</span>
                    <h2 style={{ fontFamily: SERIF, fontSize: 44, fontWeight: 300, lineHeight: '63px', color: '#1A1A1A' }}>{artisan.name} {artisan.title}</h2>
                    <span style={{ fontFamily: SANS, fontSize: 12, letterSpacing: '1px', color: '#AAAAAA' }}>{artisan.role}</span>
                    <div style={{ width: 32, height: 1, backgroundColor: '#CCCCCC' }} />
                    <p style={{ fontFamily: SANS, fontSize: 14, lineHeight: 1.9, color: '#555555', maxWidth: 680 }}>{artisan.description}</p>
                  </div>
                </ScrollReveal>
              </div>
            </section>
        ...
        {/* Certification */}
        <section style={{ backgroundColor: '#0d0c0a', padding: '80px 0' }} className="flex flex-col items-center text-center">
          <ScrollReveal className="flex flex-col items-center" style={{ maxWidth: 600 }}>
            ...
            <h3 style={{ fontFamily: SERIF, fontSize: 40, fontWeight: 300, color: '#FFFFFF', marginTop: 24 }}>경기무형문화재 제36호</h3>
      ...
      <div className="md:hidden" style={{ paddingTop: 56 }}>
          ...
          return (
            <section key={artisan.generation} style={{ backgroundColor: bg }}>
              ...
              <div style={{ padding: '40px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                <span style={{ fontFamily: SANS, fontSize: 10, letterSpacing: 3, color: '#AAAAAA' }}>{artisan.generationEn}</span>
                <h2 style={{ fontFamily: SERIF, fontSize: 32, fontWeight: 300, color: '#1A1A1A' }}>{artisan.name} {artisan.title}</h2>
                <span style={{ fontFamily: SANS, fontSize: 12, letterSpacing: 1, color: '#AAAAAA' }}>{artisan.role}</span>
                <div style={{ width: 32, height: 1, backgroundColor: '#CCCCCC' }} />
                <p style={{ fontFamily: SANS, fontSize: 14, lineHeight: 1.8, color: '#555555' }}>{artisan.description}</p>
              </div>
            </section>
          );
        })}

        {/* Certification — #0d0c0a, padding [64,24], 중앙 정렬 */}
        <section className="flex flex-col items-center text-center" style={{ backgroundColor: '#0d0c0a', padding: '64px 24px', gap: 24 }}>
          ...
          <h3 style={{ fontFamily: SERIF, fontSize: 28, fontWeight: 300, color: '#FFFFFF' }}>경기무형문화재 제36호</h3>
